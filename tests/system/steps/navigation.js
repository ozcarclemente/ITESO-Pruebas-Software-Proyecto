...
When("I open the user dropdown menu", async function () {
    ...
    await expect(page.locator('.dropdown-menu[style*="display: block"]')).toBeVisible();
});
...
Then("I should not see the authenticated navigation links", async function () {
    await expect(page.locator('.nav-link:has-text("New Article")')).toHaveCount(0);
...
When("I navigate to an invalid article page", async function () {
    await page.goto(`${APP_URL}/#/article/this-article-does-not-exist-system-test`);
});

When("I navigate to an invalid profile page", async function () {
    await page.goto(`${APP_URL}/#/profile/this-user-does-not-exist-system-test`);
});
...
Then("I should see the not found home link", async function () {
    await expect(page.locator('.not-found a:has-text("Go to home page")')).toBeVisible();
});
...
Then("I should be back on the home page from the not found page", async function () {
