...
Then('debo ser redirigido al inicio', async function () {
  ...
  if (!currentUrl.endsWith('/#/')) {
    throw new Error(`Esperaba estar en el inicio, pero la URL es: ${currentUrl}`);
  }
});
...
Then('debo ver mi nombre de usuario en la navegación', async function () {
  ...
  if (!isVisible) {
    throw new Error('No se encontró el enlace "New Article" en la barra de navegación. La sesión no se inició correctamente.');
  }
});

Then('debo ver un mensaje de error indicando credenciales inválidas', async function () {
    // En lugar de isVisible, obligamos a Playwright a esperar hasta que React renderice el error
    await page.waitForSelector('.error-messages', { state: 'visible', timeout: 5000 });
  });

// ==========================================
// ESCENARIO: SETTINGS
// ==========================================
Given('que he iniciado sesión con el usuario {string} y la contraseña {string}', async function (email, password) {
  await page.goto('http://localhost:3000/#/login');
  await page.fill('input[name="email"]', email);
  await page.fill('input[name="password"]', password);
  await page.click('button:has-text("Login")');
...
Then('debo ver los cambios reflejados o ser redirigido a mi perfil', async function () {
    ...
    if (bioValue !== 'Esta es mi nueva biografía de prueba') {
      throw new Error(`Los cambios no persistieron en la BD. Valor actual: ${bioValue}`);
    }
  });